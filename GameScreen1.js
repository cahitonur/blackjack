...
exports = Class(ImageView, function (supr) {
	...
	this.game_reset = function () {
		...
		this._stand.style.x = (baseWidth / 2) - 150;
		this._stand.style.y = pcard_y;
		this._stand.style.width = 300;
...
function hit () {
	...
	if (split) {
		if (split1) {
			...
			// get position of last card
			if (player_split_1.length == 2) {
				p1_index = that.getSubviews().slice(-3)[0].getPosition();
			}
			else {
				p1_index = that.getSubviews().slice(-2)[0].getPosition();
			}
			// show card
			that.addSubview(player_split_1.slice(-1)[0].image(p1_index.x + 30, pcard_y));
			...
			that.addSubview(player_split_2.slice(-1)[0].image(p1_index.x + 60, pcard_y));
...
function stand () {
	...
	if (split) {
		if (split1) {
        	...
        	animate(first_card).now({x: 30}, 700);
        	for (var i = good_cards.length - 1; i >= 0; i--) {
        		animate(good_cards[i]).now({x: (i * 30) + 30}, 700);
        	};
        	that.playerhand.setText(handValue(player_split_2).toString());
        	// button arrangement
        	that._hit.show();
				...
				var takeNextCardOrFinish = function() {
					...
					if (computer_cards.length < 5 && dealerTotal < 17) {
						...
						if (computer_cards.length == 3){
							d1_index = that.getSubview(8).getPosition();
							that.addSubview(computer_cards.slice(-1)[0].image(d1_index.x + 30, dcard_y));
						}
						else {
							var last_image = that.getSubviews().slice(-2)[0].getPosition();
							that.addSubview(computer_cards.slice(-1)[0].image(last_image.x + 30, dcard_y));
						}
        				...
        				if (computer_cards.length == 2 && player_split_2.length == 2 && handValue(player_split_2) == 21) {
        					...
        					split_status2.setText("You win!");
        				}
        				else {
        					split_status2.setText("Dealer win!");
        				}
            			gameon = false;
            			that._hit.hide();
            			that._stand.hide();
		            	...
		            	split_status2.setText("D busted, U win!");
		            	gameon = false;
            			that._hit.hide();
            			that._stand.hide();
            			that._deal.style.visible = true;
        		...
        		var checkSecondGroup = function () {
					...
					if (handValue(player_split_1) > 21) {
        				...
        				if (computer_cards.length == 2 && player_split_1.length == 2 && handValue(player_split_1) == 21) {
        					...
        					split_status1.setText("You win!");
        				}
        				else {
        					split_status1.setText("Dealer win!");
        				}
        			}
        			else if (dealerTotal > 21) {
		            	split_status1.setText("d busted, u win!");
        			}
        			else {
